...
import {
  Search, ChevronDown, ChevronUp, Video, Clock, CheckCircle,
...
import ClassesCalendarView from "./ClassesCalendarView";
import { submitClassReport } from '../../api/classReports';
...
const ClassesPage = () => {
  ...
  const [deleteExecutor, setDeleteExecutor] = useState(null);
  ...
  }, [searchTerm, globalFilter, sortBy, sortOrder, statusFilter, teacherFilter, guardianFilter, tabFilter, currentPage, isAdminUser]);
  ...
  // Use a ref to avoid referencing fetchClasses before it's initialized (TDZ)
    ...
    useEffect(() => {
      if (process.env.REACT_APP_PUBLIC_SITE_URL) return;
      if (typeof window === 'undefined') return;
      setPublicEvaluationLink(resolvePublicEvaluationLink());
    }, []);
  useEffect(() => {
    const handler = () => {
      try {
  ...
  useEffect(() => {
    if (!evaluationLinkCopied) return undefined;
    const timer = setTimeout(() => setEvaluationLinkCopied(false), 2000);
    return () => clearTimeout(timer);
  }, [evaluationLinkCopied]);
  ...
  }, [fetchTeacherAvailability, shareMode, showShareModal, teachers]);
...
Would you like to create another series anyway?`
  ...
  const handleUpdateClass = async (e) => {
    ...
    try {
      ...
      if (editClass.isRecurring) {
        ...
        // Prevent accidental empty recurrence which backend might interpret as deletion
  ...
  const handleSubmitReport = async (e) => {
    e.preventDefault();
    try {
  const res = await api.put(`/classes/${reportClass._id}/report`, classReport);
      if (res.data.message) {
      ...
      alert(err.response?.data?.message || "Failed to submit class report");
      console.error(err);
    }
  };
  ...
  const handleDeleteCountdownStart = useCallback((scope, executeDeleteFn) => {
    // Store the scope and executor function
    setDeleteScope(scope);
    setDeleteExecutor(() => executeDeleteFn);
  ...
  const formatDateTime = (dateString, classTimezone = DEFAULT_TIMEZONE) => {
  ...
  }, [meetings, user?.timezone]);

  const meetingsForActiveTab = tabFilter === 'previous'
    ? meetingBuckets.past
    : meetingBuckets.upcoming;
  const meetingListLimit = isAdminUser ? 4 : 1;
  const highlightedMeetings = meetingsForActiveTab.slice(0, meetingListLimit);
  ...
  const handleCopyPublicEvaluationLink = () => {
    ...
    try {
      if (typeof navigator !== 'undefined' && navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(publicEvaluationLink)
          .then(() => setEvaluationLinkCopied(true))
          .catch((err) => {
            console.error('Failed to copy evaluation link', err);
            setEvaluationLinkCopied(false);
          });
        return;
      }
    } catch (err) {
                ...
                <div className="space-y-5">
                  ...
                  <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-amber-900">Public evaluation booking page</p>
                        <p className="text-xs text-amber-700">Share this link with new families so they can self-schedule.</p>
                      </div>
                      <button
                        type="button"
                        onClick={openPublicEvaluationPage}
                        className="inline-flex items-center gap-1 rounded-full border border-amber-300 px-3 py-1 text-xs font-semibold text-amber-900 hover:bg-amber-100"
                      >
                        <ExternalLink className="h-3.5 w-3.5" />
                        Open
                      </button>
                    </div>
                    <div className="mt-3 flex items-center gap-2 rounded-xl bg-white px-3 py-2 shadow-inner">
                      <span className="flex-1 truncate text-xs font-mono text-amber-900">{publicEvaluationLink}</span>
                      <button
                        type="button"
                      ...
                      >
                        ...
                        {evaluationLinkCopied ? 'Copied' : 'Copy'}
                      </button>
                    </div>
                  </div>
