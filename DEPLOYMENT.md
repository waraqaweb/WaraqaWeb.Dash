...
Never commit credentials (JWT secrets, SMTP passwords, admin passwords, tokens). If anything sensitive was pushed, rotate it immediately.
...
# Step 6.1 (DROPLET): Faster deploys (recommended)

Use the built-in deploy helper to make updates faster and safer.
...
Force full rebuild (slow; useful after Dockerfile/infra changes):
...
Optional fastest mode (pull CI-built images from GHCR):

- Enable `.github/workflows/build-images.yml` in GitHub Actions
- Set GitHub repo variables:
  - `REACT_APP_API_URL` (example: `https://your-domain/api`)
  - `REACT_APP_SOCKET_URL` (example: `https://your-domain`)
...
OLD_SHA="$(cat /opt/waraqa/.previous_deploy_sha)"
...
# Rollbacks are usually backend-only, but if unsure you can add `frontend nginx`.
...
git status
git add -A
git commit -m "Update - improvements1"
git push origin main
```
...
IMPORTANT: Only copy/paste the code block below into the droplet shell.
Do NOT paste any surrounding explanation text (e.g. lines like “then it computes…”).
...
# - Avoid BuildKit provenance/attestation work (can be slow / look stuck)
# - Rebuild ONLY services affected by the code change (avoids slow Vite rebuilds)
export DOCKER_BUILDKIT=1
export COMPOSE_DOCKER_CLI_BUILD=1
export BUILDX_NO_DEFAULT_ATTESTATIONS=1
...
# If nginx templates / compose changed, rebuild nginx
if printf "%s\n" "$CHANGED" | grep -qE '^(deploy/nginx/|docker-compose\.yml)'; then
	SERVICES="$SERVICES nginx"
fi

# If backend code changed, rebuild backend (nginx will pick it up)
if printf "%s\n" "$CHANGED" | grep -qE '^(backend/|deploy/backend/)'; then
	SERVICES="$SERVICES backend"
fi

# If frontend code or its deploy config changed, rebuild frontend
if printf "%s\n" "$CHANGED" | grep -qE '^(frontend/|deploy/frontend/)'; then
	SERVICES="$SERVICES frontend"
fi

# Rebuild/restart (minimal downtime)
if [ -n "$(printf "%s" "$SERVICES" | tr -d ' ')" ]; then
	echo "Rebuilding services:$SERVICES"
	docker compose up -d --build $SERVICES
else
...
# nginx changes are most likely here; if unsure just rebuild nginx+backend.
