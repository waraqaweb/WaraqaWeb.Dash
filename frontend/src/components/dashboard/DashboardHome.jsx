...
import PendingReportsList from './widgets/PendingReportsList';
import FirstClassReminder from './widgets/FirstClassReminder';
import ChartCard from './widgets/ChartCard';
import DashboardChartCard from './widgets/DashboardChartCard';
import {
  ResponsiveContainer,
  LineChart,
  Line,
...
const DashboardHome = () => {
  ...
  const fetchStats = React.useCallback(async () => {
    ...
    try {
      ...
      const role = payload.role ?? (user && user.role) ?? null;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const roleLabel = (stats.role || user?.role || 'unknown');
  ...
  const createdAtGlobal = user?.createdAt ? new Date(user.createdAt) : null;
  ...
  const renderAdminDashboard = () => {
            ...
            let totalUsers = 0, totalTeachers = 0, totalGuardians = 0, totalStudents = 0;
            ...
            // guardians & students (best-effort)
            totalGuardians = pickFirst([
              data.users?.byRole?.guardian,
              data.users?.guardian,
              data.summary?.users?.byRole?.guardian,
              data.guardiansCount
            ]);
            totalStudents = pickFirst([
              data.users?.byRole?.student,
              data.users?.student,
              data.summary?.users?.byRole?.student,
              data.studentsCount
            ]);
            ...
            let classesCompletedThisMonth = 0;
            let cancellationsThisMonth = 0;
            let classesWithoutReportsCount = 0;
            let pendingReschedulesCount = 0;
            if (Array.isArray(data.classes)) {
              totalClasses = data.classes.reduce((s, c) => s + (c.count ?? 0), 0);
              ...
              classesCompletedThisMonth = data.classes.completedThisMonth ?? data.classes.completedThisMonthCount ?? 0;
              cancellationsThisMonth = data.classes.cancelledThisMonth ?? data.classes.cancellationsThisMonth ?? 0;
              classesWithoutReportsCount = data.classes.classesWithoutReportsCount ?? 0;
              pendingReschedulesCount = data.classes.pendingReschedulesCount ?? 0;
            }

            const activeClasses = data.classes?.activeCount ?? totalClasses ?? classesNext7Days ?? 0;
  ...
  const renderGuardianDashboard = () => {
    ...
    const monthlyBillDisplay = (() => {
      try {
        if (lastPaid && (lastPaid.hours != null)) {
          const from = lastPaid.fromDate ? formatClassDate(lastPaid.fromDate) : '—';
          return `Last paid: ${lastPaid.hours} hrs from ${from}`;
        }
        return 'No paid hours found';
      } catch (e) {
        return 'No paid hours found';
      }
    })();
