...
const TeacherSalaries = () => {
  ...
  // Arabic month names for WhatsApp message
  ...
  const buildTeacherMessage = (invoice, publicLink) => {
    ...
    lines.push('', 'نسأل الله أن يبارك في وقتكم وعلمكم، وأن يجزيكم عنا خير الجزاء.', '', 'وجزاكم الله خيرا');
    return lines.join('\n');
  };

  const getInvoicePublicLink = (invoice) => {
    const slug = invoice?.invoiceSlug || invoice?.shareToken;
    return slug ? `${window.location.origin}/dashboard/teacher-salary/shared/${slug}` : '';
  };

  const handleCopyPublicLink = (invoice) => {
    const url = getInvoicePublicLink(invoice);
    if (!url) return;
    navigator.clipboard.writeText(url).then(() => {
      setCopiedLinkId(invoice._id);
      setTimeout(() => setCopiedLinkId(null), 2000);
    });
  };

  const handleOpenWhatsapp = (invoice) => {
    const phone = String(invoice?.teacher?.phone || invoice?.teacher?.whatsapp || '').replace(/\D/g, '').replace(/^0+/, '');
    if (!phone) {
      alert('No phone number found for this teacher.');
      return;
    }
    const publicLink = getInvoicePublicLink(invoice);
    const msg = buildTeacherMessage(invoice, publicLink);
    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(msg)}`, '_blank', 'noopener,noreferrer');
  };

  const openTeacherMsgModal = (invoice) => {
    const publicLink = getInvoicePublicLink(invoice);
    setTeacherMsgModal({ invoice, message: buildTeacherMessage(invoice, publicLink), copied: false });
  };
  ...
  const handleSendTeacherWhatsapp = () => {
    if (!teacherMsgModal) return;
    const phone = String(
      teacherMsgModal.invoice?.teacher?.phone ||
      teacherMsgModal.invoice?.teacher?.whatsapp ||
      ''
                    ...
                    <div key={invoice._id} className={`rounded-xl border border-slate-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${selected ? 'ring-2 ring-primary/30 bg-primary/5' : ''}`}>
                      <div className="flex flex-col gap-3 p-3 md:p-4 lg:flex-row lg:items-start lg:justify-between">
                            ...
                            <div className="flex flex-wrap items-center gap-4">
                              ...
                              {(() => {
                                ...
                                return (
                                  <>
                                    {premiumUSD !== 0 && (
                                      <span className="text-xs font-semibold text-orange-600">Premium: ${premiumUSD.toFixed(2)}</span>
                                    )}
                                    {otherUSD !== 0 && (
                                      <span className={`text-xs font-semibold ${otherUSD > 0 ? 'text-sky-600' : 'text-red-500'}`}>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {/* Copy public link */}
                          {(invoice.invoiceSlug || invoice.shareToken) && (
                            <button
                              onClick={() => handleCopyPublicLink(invoice)}
                              className={`inline-flex items-center justify-center rounded-md p-2 transition ${copiedLinkId === invoice._id ? 'text-emerald-600 bg-emerald-50' : 'text-slate-600 hover:bg-slate-100'}`}
                              type="button"
                              title={copiedLinkId === invoice._id ? 'Link copied!' : 'Copy public link'}
                            >
                              {copiedLinkId === invoice._id ? <CheckCheck className="h-4 w-4" /> : <Link2 className="h-4 w-4" />}
                            </button>
                          )}
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center rounded-md p-2 text-green-600 hover:bg-green-50 hover:text-green-700 transition"
                            type="button"
                          ...
                          {/* Compose message */}
                          <button
                            onClick={() => openTeacherMsgModal(invoice)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setTeacherMsgModal(null)}>
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {teacherMsgModal.invoice?.teacher?.firstName} {teacherMsgModal.invoice?.teacher?.lastName}
                  {teacherMsgModal.invoice?.month ? ` — ${arabicMonths[teacherMsgModal.invoice.month - 1]} ${teacherMsgModal.invoice.year || ''}` : ''}
                </p>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4">
              ...
              >
                {teacherMsgModal.copied ? <CheckCheck className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4" />}
