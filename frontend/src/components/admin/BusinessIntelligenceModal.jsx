...
import { ResponsiveContainer, LineChart, BarChart, ComposedChart, Line, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Cell } from 'recharts';
...
const SECTIONS = [
  { id: 1,  label: 'Critical Numbers',    icon: BarChart3   },
  { id: 2,  label: 'Target Tracker',      icon: Target      },
  { id: 3,  label: 'Student Health',      icon: Users       },
  { id: 4,  label: 'Guardian Health',     icon: ShieldCheck },
  { id: 5,  label: 'Teacher Performance', icon: Award       },
  { id: 6,  label: 'Financial',           icon: DollarSign  },
  { id: 7,  label: 'Vacations',           icon: Globe       },
  { id: 8,  label: 'Growth',              icon: TrendingUp  },
  { id: 9,  label: 'Owner Insights',      icon: Lightbulb   },
  { id: 10, label: 'History Table',       icon: Layers      },
  { id: 11, label: 'Scoreboard',          icon: Activity    },
];
...
const N = (v, d = 1) => { const n = Number(v); return Number.isFinite(n) ? n.toFixed(d) : 'e2e'; };
const $$ = v => { const n = Number(v); return Number.isFinite(n) ? `$${n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'e2e'; };
const PCT = v => { const n = Number(v); return Number.isFinite(n) ? `${n.toFixed(1)}%` : 'e2e'; };
...
const applyWhatIf = (critical, wi) => {
  ...
  const np   = gp - own - oth;
  return { completedHours: hrs, revenue: rev, teacherSalaries: tSal, paypalFees: pp, grossProfit: gp,
    ownerSalary: own, otherExpenses: oth, netProfit: np,
    grossMargin: rev > 0 ? (gp / rev) * 100 : 0, netMargin: rev > 0 ? (np / rev) * 100 : 0,
    revenuePerHour: rph, teacherCostPerHour: tcph,
    profitPerHour: hrs > 0 ? gp / hrs : 0, netProfitPerHour: hrs > 0 ? np / hrs : 0,
    activeStudents: critical.activeStudents, activeTeachers: critical.activeTeachers,
    activePayingGuardians: critical.activePayingGuardians, prev: critical.prev };
};
...
const generateInsights = (computed, data, targets) => {
  ...
  if (sh?.retentionRate < 85 && sh?.lostStudents > 0) insights.push({ type: 'action', label: 'Students at risk', text: `${sh.lostStudents} inactive students — check for disengagement` });
  const actions = [];
  if (targets.hours && c.completedHours < targets.hours * 0.8) actions.push(`Only ${N(c.completedHours / targets.hours * 100, 0)}% of hours target — ${N(targets.hours - c.completedHours, 0)} hrs needed`);
  if (targets.revenue && c.revenue < targets.revenue * 0.8) actions.push(`Revenue at ${N(c.revenue / targets.revenue * 100, 0)}% of target`);
...
function TrendBadge({ curr, prev }) {
  ...
  const up = d > 0; const Icon = up ? TrendingUp : d < 0 ? TrendingDown : Minus;
  const cls = up ? 'text-emerald-600' : d < 0 ? 'text-rose-500' : 'text-muted-foreground';
  return <span className={`inline-flex items-center gap-0.5 text-[11px] font-semibold ${cls}`}><Icon className="h-3 w-3" />{up ? '+' : ''}{N(d, 1)}% vs prev</span>;
}
    ...
    <div className={`rounded-xl border border-border bg-card p-2.5 flex flex-col gap-0.5 ${className}`}>
      ...
      {target != null && (
        <div className="flex items-center gap-1 mt-0.5">
          ...
          {onEditTarget && <button type="button" onClick={onEditTarget} className="ml-auto p-0.5 text-muted-foreground hover:text-foreground rounded"><Edit3 className="h-3 w-3" /></button>}
        </div>
      )}
    </div>
  );
...
function ProgressBar({ value, target, label, sub }) {
  ...
  return (
    <div className="space-y-1">
      {label && <div className="flex items-center justify-between text-xs"><span className="font-medium">{label}</span><span className="text-muted-foreground">{sub || `${value} / ${target}`}</span></div>}
...
function InlineEdit({ value, onSave, prefix = '', suffix = '' }) {
  ...
  if (editing) return (
    <span className="inline-flex items-center gap-1">
      {prefix}<input type="number" value={draft} onChange={e => setDraft(e.target.value)}
        onBlur={save} onKeyDown={e => { if (e.key === 'Enter') save(); if (e.key === 'Escape') setEditing(false); }}
        className="w-20 rounded border border-primary px-1 py-0.5 text-xs bg-background" autoFocus />{suffix}
      <button type="button" onClick={save} className="text-emerald-600"><Check className="h-3 w-3" /></button>
    </span>
  );
  return <button type="button" onClick={open} className="text-xs text-muted-foreground hover:text-foreground border-b border-dashed border-muted-foreground">{prefix}{value}{suffix}</button>;
}
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width="80" height="80" viewBox="0 0 80 80" className="-rotate-90">
        ...
        <circle cx="40" cy="40" r={r} strokeWidth="8" fill="none" className={ring} strokeDasharray={circ} strokeDashoffset={circ * (1 - s / 100)} strokeLinecap="round" />
      </svg>
...
function HistoryBar({ data, dataKey, color = '#6366f1', height = 80 }) {
  ...
  return <div style={{ height }}><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}><Bar dataKey={dataKey} fill={color} radius={[2, 2, 0, 0]} /><Tooltip contentStyle={{ fontSize: 10 }} /></BarChart></ResponsiveContainer></div>;
}

function SH({ title, sub }) {
  return <div className="mb-4"><h2 className="text-base font-semibold">{title}</h2>{sub && <p className="text-xs text-muted-foreground mt-0.5">{sub}</p>}</div>;
}

// ─── Section 1 ────────────────────────────────────────────────────────────────
function S1Critical({ computed, data, targets, setTargets, wi, setWI, wiMode, setWIMode }) {
  const c = computed; const prev = c.prev || {}; const hist = data.monthlyHistory || [];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
        <div className="flex items-center justify-between mb-3">
          <div><div className="text-sm font-semibold text-amber-900">What-if calculator</div><div className="text-xs text-amber-700">Adjust inputs to see projected outcomes.</div></div>
        ...
        {wiMode && <button type="button" onClick={() => { setWI({ ownerSalary: 0, otherExpenses: 0, paypalRate: 3.5 }); setWIMode(false); }} className="mt-2 text-xs text-amber-700 underline">Reset to actuals</button>}
      </div>
      <Grid cols={4}>
      ...
      {wiMode && (
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
          <div className="text-sm font-semibold text-amber-900 mb-3">What-if result vs actuals</div>
          <Grid cols={4}>
            {[['Projected revenue', $$(c.revenue)], ['Projected gross profit', $$(c.grossProfit)], ['Net profit', $$(c.netProfit ?? 0)], ['Gross margin', PCT(c.grossMargin)]].map(([l, v]) => (
              <div key={l} className="rounded-xl bg-white border border-amber-200 p-2">
                <div className="text-[10px] text-amber-700 uppercase tracking-wide">{l}</div>
                <div className="text-lg font-bold text-amber-900 mt-0.5">{v}</div>
              </div>
            ))}
          </Grid>
          <div className="mt-2 text-xs text-amber-700">vs actual: revenue {$$(actual.revenue)}, gross profit {$$(actual.grossProfit)}</div>
        </div>
      )}
    </div>
  );
...
// ─── Section 2 ────────────────────────────────────────────────────────────────
function S2Targets({ computed, targets, setTargets }) {
  const c = computed;
  const ut = (k, v) => { const t = { ...targets, [k]: v }; setTargets(t); saveTargets(t); };
  const rows = [
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {rows.map(({ key, label, value, prefix, suffix }) => (
          <div key={key} className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium">{label}</span>
              <span className="text-xs text-muted-foreground">Target: <InlineEdit value={targets[key] || 0} prefix={prefix} suffix={suffix} onSave={v => ut(key, v)} /></span>
            </div>
            {value != null && targets[key] > 0 && <ProgressBar value={+N(value, 1)} target={targets[key]} sub={`${prefix}${N(value, 1)} / ${prefix}${targets[key]}${suffix}`} />}
...
// ─── Section 3 ────────────────────────────────────────────────────────────────
function S3StudentHealth({ data, targets, setTargets }) {
  const sh = data.studentHealth; const hist = data.monthlyHistory || [];
  const ut = (k, v) => { const t = { ...targets, [k]: v }; setTargets(t); saveTargets(t); };
  return (
    <div className="space-y-5">
      <SH title="Student Health" />
      <Grid cols={4}>
        <KPICard label="Active students" value={sh.activeStudents} target={targets.activeStudents} highlight={sh.activeStudents >= (targets.activeStudents || 0)} onEditTarget={() => { const v = prompt('Active students target:', targets.activeStudents); if (v) ut('activeStudents', Number(v)); }} />
      ...
      {hist.length > 1 && <div className="rounded-2xl border border-border bg-card p-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-3">Active students · 12 months</div><HistoryBar data={hist} dataKey="activeStudents" color="#6366f1" height={100} /></div>}
    </div>
  );
}

// ─── Section 4 ────────────────────────────────────────────────────────────────
function S4GuardianHealth({ data }) {
  const gh = data.guardianHealth; const hist = data.monthlyHistory || [];
  return (
    <div className="space-y-5">
          ...
          {[['Top 10 by revenue', gh.topByRevenue, g => $$(g.revenue)], ['Top 10 by hours', gh.topByHours, g => `${N(g.hours, 1)} hrs`]].map(([title, list, fmt]) => (
            <div key={title} className="rounded-2xl border border-border bg-card overflow-hidden">
              ...
              <div className="divide-y divide-border">
                {list.map((g, i) => <div key={i} className="flex items-center justify-between px-4 py-2 text-sm hover:bg-muted/10"><span className="font-medium">{i + 1}. {g.name}</span><span className="font-semibold text-emerald-600">{fmt(g)}</span></div>)}
              </div>
            </div>
          ))}
        </div>
      )}
      {hist.length > 1 && <div className="rounded-2xl border border-border bg-card p-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-3">Active guardians · 12 months</div><HistoryBar data={hist} dataKey="activeGuardians" color="#22c55e" height={80} /></div>}
      <Grid cols={2}>
...
// ─── Section 5 ────────────────────────────────────────────────────────────────
function S5TeacherPerformance({ data }) {
  const tp = data.teacherPerformance; const hist = data.monthlyHistory || [];
        ...
        <div className="rounded-2xl border border-border bg-card overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-border bg-muted/20">{['Teacher','Hours','Classes','Students','Rate/hr','Est. cost'].map(h => <th key={h} className={`px-3 py-2 font-medium text-muted-foreground ${h==='Teacher'?'text-left':'text-right'}`}>{h}</th>)}</tr></thead>
              <tbody>
                {teachers.map((t, i) => (
              ...
              <tfoot><tr className="border-t-2 border-border bg-muted/20 font-semibold"><td className="px-3 py-2 text-muted-foreground">Totals</td><td className="px-3 py-2 text-right">{N(tp.totalTeacherHours,1)}</td><td className="px-3 py-2 text-right">{teachers.reduce((s,t)=>s+t.classes,0)}</td><td className="px-3 py-2 text-right">—</td><td className="px-3 py-2 text-right text-muted-foreground">{$$(tp.avgHourlyRate)} avg</td><td className="px-3 py-2 text-right">{$$(tp.salaryCost)}</td></tr></tfoot>
            </table>
          </div>
        </div>
      )}
      {hist.length > 1 && <div className="rounded-2xl border border-border bg-card p-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-3">Completed hours · 12 months</div><HistoryBar data={hist} dataKey="completedHours" color="#8b5cf6" height={80} /></div>}
    </div>
  );
}

// ─── Section 6 ────────────────────────────────────────────────────────────────
function S6Financial({ computed, data, targets, setTargets }) {
  const c = computed; const fin = data.financial; const hist = data.monthlyHistory || [];
  const own = c.ownerSalary ?? 0; const oth = c.otherExpenses ?? 0;
  const np = c.grossProfit - own - oth; const netMargin = c.revenue > 0 ? (np / c.revenue) * 100 : 0;
  return (
    <div className="space-y-5">
        ...
        <div className="divide-y divide-border text-sm">
          {[
            ...
            <div key={l} className={`flex items-center justify-between px-4 py-2.5 ${border?'border-t-2 border-border bg-muted/10':''}`}>
              <span className="text-muted-foreground">{l}</span><span className={cls}>{$$(v)}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4 text-sm text-amber-900">
        <span className="font-semibold">Vacation hours lost</span> requires per-schedule analysis. The system tracks active vacation counts as a proxy. Detailed lost-hours reporting can be added with schedule data.
      </div>
    </div>
  );
}
...
// ─── Section 8 ────────────────────────────────────────────────────────────────
function S8Growth({ data }) {
  const g = data.growth; const hist = data.monthlyHistory || []; const sh = data.studentHealth;
  return (
    <div className="space-y-5">
          ...
          <div className="h-48"><ResponsiveContainer width="100%" height="100%">
            <BarChart data={hist} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="revenue" radius={[3, 3, 0, 0]} name="Revenue">
                {hist.map((mo, i) => { const prev = hist[i-1]?.revenue||0; return <Cell key={i} fill={!prev||mo.revenue>=prev?'#22c55e':'#ef4444'} />; })}
              </Bar>
            </BarChart>
          </ResponsiveContainer></div>
        </div>
...
function S9Insights({ insights }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {!insights.length && <div className="text-muted-foreground text-sm">Load data to generate insights.</div>}
      {insights.map((ins, i) => {
        const { Icon, bg, border, title, text } = styles[ins.type] || styles.explain;
        return (
          <div key={i} className={`rounded-2xl border ${border} ${bg} p-4`}>
            ...
            {ins.list && <ul className={`mt-1 space-y-1 text-sm ${text}`}>{ins.list.map((item, j) => <li key={j} className="flex items-start gap-1.5"><span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-60" />{item}</li>)}</ul>}
          </div>
        );
      })}
    </div>
...
function S10History({ data }) {
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement('a'); a.href = `data:text/csv;charset=utf-8,${encodeURIComponent(csv)}`; a.download = `waraqa-history-${new Date().toISOString().slice(0,7)}.csv`; a.click();
  };
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between"><SH title="Monthly History" sub="Last 12+ months · completed hours only" /><button type="button" onClick={exportCSV} className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium hover:bg-muted/50"><Download className="h-3.5 w-3.5" /> Export CSV</button></div>
      {!hist.length ? <div className="text-muted-foreground text-sm">No history data.</div> : (
        <div className="overflow-x-auto rounded-2xl border border-border">
          <table className="w-full text-xs whitespace-nowrap">
            <thead><tr className="border-b border-border bg-muted/30">{['Month','Hrs','Revenue','Teacher cost','PayPal','Gross profit','Students','Guardians','Teachers','New','Avg hrs','Rev/hr','Profit/hr'].map(h=><th key={h} className="px-3 py-2 text-left font-medium text-muted-foreground">{h}</th>)}</tr></thead>
            <tbody>
...
// ─── Section 11 ───────────────────────────────────────────────────────────────
function S11Scoreboard({ computed, data, targets }) {
  const sb = data.scoreboard; if (!sb) return null;
  const c = computed;
  const overallColor = sb.overall>=75?'text-emerald-600':sb.overall>=50?'text-amber-500':'text-rose-500';
  const label = sb.overall>=80?'Excellent':sb.overall>=65?'Good':sb.overall>=50?'Fair':'Needs work';
  return (
    <div className="space-y-6">
      <SH title="Business Health Scoreboard" sub="0–100 each — based on actual vs previous period" />
      <div className="rounded-2xl border border-border bg-card p-6 text-center">
        <div className={`text-6xl font-black ${overallColor}`}>{sb.overall}</div>
        <div className="text-2xl font-semibold text-muted-foreground mt-1">{label}</div>
        <div className="text-sm text-muted-foreground">Overall business health score</div>
      </div>
      <div className="grid grid-cols-5 gap-4">
        {[['Hours',sb.hours],['Revenue',sb.revenue],['Profit',sb.profit],['Retention',sb.retention],['Growth',sb.growth]].map(([l,s]) => <ScoreGauge key={l} score={s} label={l} />)}
      </div>
      <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
        <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Score breakdown</div>
        {[
          ...
          <div key={label} className="flex items-center gap-3">
            ...
            <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden"><div className={`h-full rounded-full ${score>=75?'bg-emerald-500':score>=50?'bg-amber-400':'bg-rose-400'}`} style={{width:`${score}%`}} /></div>
...
function PeriodPicker({ period, setPeriod, customRange, setCustomRange }) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <select value={period} onChange={e => setPeriod(e.target.value)} className="rounded-lg border border-border bg-background px-2 py-1 text-xs font-medium">
...
export default function BusinessIntelligenceModal({ open, onClose }) {
  ...
  useEffect(() => { if (open) return fetchData(); }, [open, period, customRange]);
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-border bg-background shrink-0 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          ...
          <button type="button" onClick={fetchData} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground"><RefreshCcw className="h-4 w-4" /></button>
          <button type="button" onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
        </div>
      </div>
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 bg-background">
          <select value={section} onChange={e => setSection(Number(e.target.value))} className="md:hidden w-full rounded-xl border border-border bg-background px-3 py-2 text-sm font-medium mb-4">
            {SECTIONS.map(s => <option key={s.id} value={s.id}>{s.id}. {s.label}</option>)}
          </select>
          {error && <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800 mb-4"><AlertTriangle className="inline h-4 w-4 mr-1.5 mb-0.5" />{error}</div>}
          {loading && !data && <div className="flex items-center justify-center h-48 text-muted-foreground text-sm"><RefreshCcw className="h-5 w-5 animate-spin mr-2" />Loading data…</div>}
          {data && computed && <>
            {section===1  && <S1Critical {...sp} />}
            {section===2  && <S2Targets {...sp} />}
            {section===3  && <S3StudentHealth data={data} targets={targets} setTargets={setTargets} />}
            {section===4  && <S4GuardianHealth data={data} />}
            {section===5  && <S5TeacherPerformance data={data} />}
            {section===6  && <S6Financial {...sp} />}
            {section===7  && <S7Vacations data={data} />}
            {section===8  && <S8Growth data={data} />}
            {section===9  && <S9Insights insights={insights} />}
            {section===10 && <S10History data={data} />}
            {section===11 && <S11Scoreboard computed={computed} data={data} targets={targets} />}
          </>}
        </main>
      </div>
    </div>
