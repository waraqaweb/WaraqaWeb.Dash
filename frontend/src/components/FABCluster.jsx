...
export default function FABCluster({ isAdmin, isTeacher, onCreate, onShare, onSeriesScanner }) {
        ...
        <div className={`flex flex-col items-end gap-3 mb-2 ${open ? '' : 'pointer-events-none'}`}>
          ...
          {isAdmin && (
            <div className={`flex items-center gap-3 transition-all duration-200 ${open ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
              ...
              <PrimaryButton
                onClick={() => { setOpen(false); onSeriesScanner && onSeriesScanner(); }}
                circle
                size="sm"
                aria-label="Series Scanner"
              >
