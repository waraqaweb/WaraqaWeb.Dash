    ...
    <div className="bg-card rounded-lg border border-border p-3 sm:p-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[140px,1fr] sm:items-stretch sm:gap-4">
        ...
        <div className="min-w-0 flex flex-col gap-2">
          <div className="text-sm font-semibold text-foreground truncate">{buildStudentLabel(nextClass)}</div>

          <div className="flex flex-wrap gap-1.5 text-[11px]">
            {nextClass.duration && (
              <span className="rounded-full bg-brand/20 px-2 py-0.5 font-semibold text-brand">{nextClass.duration} min</span>
            )}
            {nextClass.subject && (
              <span className="rounded-full bg-primary/20 px-2 py-0.5 font-semibold text-primary">{nextClass.subject}</span>
            )}
            {nextClass.previousReport?.classScore != null && (
              <span className="rounded-full bg-muted px-2 py-0.5 font-semibold text-foreground">
                Last score: {nextClass.previousReport.classScore}/5
              </span>
            )}
          </div>

          <div className="text-xs text-muted-foreground">
            Last lesson topic: <span className="text-foreground font-medium">{lastTopic || '—'}</span>
          </div>
