...
import FirstClassFeedbackModal from '../feedback/FirstClassFeedbackModal';
...
import { getTimezoneInfo, formatTimeInTimezone } from '../../utils/timezoneUtils';
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    try {
      if (!profile || !profile._id) throw new Error('Missing profile id');
      const payload = { ...profile };

      const allowedGenders = ['male', 'female'];
      if (payload.gender !== undefined && payload.gender !== null && !allowedGenders.includes(payload.gender)) {
        delete payload.gender;
      }

      const normalizePaymentMethod = (pm) => {
        if (!pm) return pm;
        if (pm === 'bank' || pm === 'wise') return 'bank_transfer';
        if (pm === 'card') return 'credit_card';
        return pm;
      };

      if (payload.guardianInfo && payload.guardianInfo.paymentMethod) {
        payload.guardianInfo.paymentMethod = normalizePaymentMethod(payload.guardianInfo.paymentMethod);
      }

      if (payload.paymentMethod !== undefined) {
        // Don't overwrite DB with an empty placeholder from the edit modal
        if (payload.paymentMethod === '') {
          delete payload.paymentMethod;
        } else {
          if (!payload.guardianInfo) payload.guardianInfo = {};
          payload.guardianInfo.paymentMethod = normalizePaymentMethod(payload.paymentMethod);
          delete payload.paymentMethod;
        }
      }
      ...
      if (payload.spokenLanguages !== undefined) {
        if (payload.role === 'teacher') {
          ...
          payload.teacherInfo.spokenLanguages = payload.spokenLanguages;
        } else if (payload.role === 'guardian') {
          if (!payload.guardianInfo) payload.guardianInfo = {};
          payload.guardianInfo.spokenLanguages = payload.spokenLanguages;
        }
        delete payload.spokenLanguages;
      }

      if (!isAdmin) delete payload.email;

      // Do not force default hourly rates for guardians here; use database values as-is

      // Bank details handling removed (bank details are deprecated in the UI)
  ...
  const totalRequired = missingFields.length + ( (profile.phone?0:1) + (profile.timezone?0:1) + (profile.dateOfBirth?0:1) + (profile.gender?0:1) + (profile.profilePicture?0:1) );
  ...
  // Minimal searchable select for country or other long lists
  const SearchableSelect = ({ options = [], value, onChange, placeholder }) => {
    ...
    useEffect(() => {
      ...
      if (typeof highlight === 'number' && highlight >= 0 && listRef.current) {
        const el = listRef.current.querySelectorAll('div')[highlight + 1]; // +1 because first child is search input wrapper
    ...
    return (
      <div className="relative">
        <input
          ...
          onChange={(e) => { setFilter(e.target.value); onChange && onChange(e.target.value); setOpen(true); }}
          onKeyDown={handleKeyDown}
          onBlur={() => setTimeout(() => setOpen(false), 150)}
          className="border rounded-lg px-3 py-2 text-base w-full"
        />
        {open && (
          <div ref={listRef} className="absolute left-0 right-0 top-full mt-1 z-40 w-full bg-white border rounded shadow max-h-40 overflow-auto">
            <div className="p-2">
              <input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Search..." className="w-full px-2 py-1 border rounded text-sm" />
            </div>
            <div>
              {filtered.map((opt, i) => (
                <div
                  key={i}
                  className={`p-2 cursor-pointer text-sm ${i === highlight ? 'bg-gray-100' : ''}`}
