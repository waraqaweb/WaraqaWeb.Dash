...
const InvoiceViewModal = ({ invoiceSlug, invoiceId, initialInvoice = null, onClose, onInvoiceUpdate, onOpenRecordPayment }) => {
  ...
  const guardianUnsettledTotal = useMemo(
    () => guardianUnsettled.reduce((s, a) => s + Math.abs(a.amountDelta || 0), 0),
    [guardianUnsettled]
  );
      ...
      <div className="relative flex w-full max-w-6xl max-h-[90vh] flex-col overflow-hidden rounded-[32px] bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
          ...
          {isAdmin && guardianUnsettled.length > 0 && (
            <div className="mx-4 mb-4 sm:mx-8 rounded-xl border border-amber-200 bg-amber-50/60 px-3 py-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-800">
                  <Sparkles className="h-3.5 w-3.5 shrink-0" />
                  Unsettled credits from paid invoices — ${guardianUnsettledTotal.toFixed(2)}
                </div>
                <button
                  type="button"
                  onClick={handleReconcileCredits}
                  disabled={reconciling}
                  className="inline-flex items-center gap-1 rounded-lg border border-amber-300 bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800 transition hover:bg-amber-200 disabled:opacity-60"
                  title="Settle credits already covered by a later class shifted into the paid invoice. Anything still owed is left untouched; guardian hours never change."
                >
                  {reconciling ? 'Reconciling…' : 'Reconcile & settle'}
                </button>
              </div>
              <div className="mt-1 space-y-0.5">
                {guardianUnsettled.map((adj) => (
                  <div key={adj._id} className="flex items-center gap-1.5 text-[11px] text-amber-700">
                    <span className="font-medium">{adj.invoiceNumber}:</span>
                    <span className="truncate">{adj.description}</span>
                    <span className="ml-auto shrink-0 font-semibold">${Math.abs(adj.amountDelta || 0).toFixed(2)}</span>
                  </div>
                ))}
              </div>
              {reconcileResult && !reconcileResult.error && (
                <div className="mt-1 text-[11px] text-amber-700">
                  ...
                  {reconcileResult.hoursChanged ? ' · ⚠ hours changed — check server logs' : ''}
                </div>
              )}
              {reconcileResult?.error && (
                <div className="mt-1 text-[11px] font-semibold text-red-600">{reconcileResult.error}</div>
              )}
            </div>
          )}
          ...
          {isAdmin && guardianUnsettled.length === 0 && reconcileResult && !reconcileResult.error && (reconcileResult.settled?.length > 0) && (
            <div className="mx-4 mb-4 sm:mx-8 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50/60 px-3 py-1.5 text-[11px] font-medium text-emerald-700">
              <CheckCircle2 className="h-3.5 w-3.5 shrink-0" />
              <span>Credits settled · no action needed</span>
            </div>
          )}
